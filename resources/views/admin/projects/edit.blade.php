    ...
    <form action="{{ route('admin.projects.update', $project) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="status" aria-label="Default select example">
                ...
                <option value="ongoing" @if (old('status', $project->status) == 'ongoing') selected @endif>Ongoing</option>
                <option value="completed" @if (old('status', $project->status) == 'completed') selected @endif>Completed</option>
                </option>
            </select>
        </div>
            ...
            <select name="type_id" id="type_id" class="form-control">
                ...
                @foreach ($types as $type)
                    <option value="{{ $type->id }}"
                        {{ old('type_id', $project->type_id) == $type->id ? 'selected' : '' }}>{{ $type->title }}</option>
                @endforeach
            </select>
        </div>
        <div class="mt-2">
            ...
            @foreach ($technologies as $technology)
                <div class="form-check form-check-inline">
                    ...
                    @if ($errors->any())
                        ...
                        <label class="form-check-label" for="technology-{{ $technology->id }}">{{ $technology->title }}
                        </label>
                    @else
                        <input class="form-check-input" type="checkbox" value="{{ $technology->id }}" name="technologies[]"
                            id="technology-{{ $technology->id }}"
        ...
        <button type="submit" class="btn btn-primary mt-4">Submit your project</button>
    </form>
@endsection
